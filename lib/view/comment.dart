      ...
      body: Column(
        children: <Widget>[
          ...
          Row(
            ...
            children: [
              Text('@${searchedPost.userHandle}:', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14),),
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        ...
                        return ListTile(
                            title: Row(mainAxisAlignment: MainAxisAlignment.start,
                              children: [
                                everyComment[index].isEmpty||currentUser.handle.compareTo(everyComment[index].substring(0, everyComment[index].indexOf(":")).replaceAll(" ", ''))!=0 ? const SizedBox(width: 46,) :IconButton(icon: const Icon(Icons.delete_forever),
                                onPressed: () async {
                                  removeComment(searchedPost.id, everyComment[index]);
                                  setState(() {
                                  });
                                },),
                                RichText(
                                  text: TextSpan(
                                    ...
                                    children: <TextSpan>[
                                      TextSpan(
                                        text: everyComment[index].substring(0, everyComment[index].indexOf(":") + 1).replaceAll(" ", '').isEmpty ?'' : '@${everyComment[index].substring(0, everyComment[index].indexOf(":") + 1).replaceAll(" ", '')}',
                                        style: const TextStyle(fontWeight: FontWeight.bold),
                                      ),
                                      TextSpan(text: everyComment[index].substring(everyComment[index].indexOf(":") + 1)),
                                    ],
                                  ),
                                )
                              ])
            ...
            child: Row(
              children: [
                ...
                IconButton(onPressed: () {
                  addComment(searchedPost.id, commentController.text, currentUser.handle);
